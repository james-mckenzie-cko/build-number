      ...
      res.on("end", () => {
        ...
        if (res.headers["content-encoding"] === "gzip") {
          ...
          callback(
            null,
            res.statusCode,
            buffer.length > 0 ? JSON.parse(buffer) : null
          );
        }
      });
...
function main() {
  ...
  if (fs.existsSync(path)) {
    ...
    console.log(
      `Build number already generated in earlier jobs, using build number ${buildNumber}...`
    );
  ...
  request(
    "GET",
    `/repos/${env.GITHUB_REPOSITORY}/git/refs/tags/${prefix}build-number-`,
    null,
    (err, status, result) => {
      ...
      if (status === 404) {
        ...
        if (nrTags.length > MAX_OLD_NUMBERS) {
          fail(
            `ERROR: Too many ${prefix}build-number- refs in repository, found ${nrTags.length}, expected only 1. Check your tags!`
          );
        }
        ...
        if (err) {
          ...
          fail(
            `Getting build-number refs failed with http status ${status}, error: ${JSON.stringify(
              result
            )}`
          );
        }
      ...
      request(
        ...
        (err, status, result) => {
          if (status !== 201 || err) {
            fail(
              `Failed to create new build-number ref. Status: ${status}, err: ${err}, result: ${JSON.stringify(
                result
              )}`
            );
          }
          ...
          console.log(
            `Successfully updated build number to ${nextBuildNumber}`
          );
          ...
          if (nrTags) {
            ...
            for (let nrTag of nrTags) {
              request(
                "DELETE",
                `/repos/${env.GITHUB_REPOSITORY}/git/${nrTag.ref}`,
                null,
                (err, status, result) => {
                  if (status !== 204 || err) {
                    console.warn(
                      `Failed to delete ref ${
                        nrTag.ref
                      }, status: ${status}, err: ${err}, result: ${JSON.stringify(
                        result
                      )}`
                    );
                  } else {
